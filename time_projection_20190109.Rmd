...
```{r fig1, fig.height = 1.5, fig.width = 3}
library(scales)
require(ggplot2)
require(gridExtra)
library(Rmisc)
...
df_hist$donor <- "NONE"
donor_levels <- if (with_donor) c("PF", "3T3", "B16") else c("FL-DC", "PF", "3T3", "B16")
...
if (!with_donor) {
  df_hist[df_hist$image %in% c("Well6_Seq0006", "Well8_Seq0004") &
          (df_hist$exp %in% c("N4-2-7", "N4-2-9")),]$donor <- "FL-DC"
}
...
if (!with_donor) {
  df_hist[df_hist$image %in% c("Well4_Seq0003", "Well8_Seq0004") &
          (df_hist$exp %in% c("N4-2-13")),]$donor <- "FL-DC"
}
...
# show plots
ggplot(data = df_hist, aes(x=donor, y=dc_foci_ratio)) +
